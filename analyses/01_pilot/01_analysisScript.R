...
### import data
data_raw <- read.csv("D:/Master/SS20/EPLab/data/01_pilot/01_raw_data.csv")
view(data_raw)

#data_rV is a table containing only relevant variables
data_rV <- data_raw %>% 
  select(submission_id, age, condition, correctness, education, 
         filler, gender, languages, reactionTimes_continuation, 
...
data_filter_el <- data_rV %>% filter(languages %in% c('C1', 'B1')) # need to adapted in final analysis
...
write.csv(data_clean, file = "D:/Master/SS20/EPLab/data/01_pilot/02_clean_data.csv")
...
rt_critical_factors %>%
  ggplot(aes(x = rt_trigger_critical)) + geom_histogram()
rt_critical_factors %>%
  ggplot(aes(x = rt_continuation_critical)) + geom_histogram()
